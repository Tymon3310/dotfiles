...
Singleton {
    ...
    // Quickshell's Networking lists Wi-Fi devices only (0.3), so the cable is
    // read from nmcli: any ethernet device NetworkManager has connected.
    // Unmanaged virtual NICs (VMware, Docker) report "unmanaged" and never count.
    property bool wiredConnected: false

    readonly property Process wiredQuery: Process {
        command: ["nmcli", "-t", "-f", "TYPE,STATE", "device"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: root.wiredConnected = text.split("\n")
                .some(line => line === "ethernet:connected")
        }
    }

    readonly property Timer wiredPoll: Timer {
        interval: 10000
        running: true
        repeat: true
        onTriggered: root.wiredQuery.running = true
    }
    ...
    readonly property string script: Quickshell.shellPath("parts/scripts/network.py")
