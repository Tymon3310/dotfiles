...
// Audio module-level poll
const audioInfo = createPoll({ vol: "0%", mic: "0%", micMuted: false }, 2000, async () => {
  try {
...
export function Audio({ openDropdown, setup }: DropdownProps) {
  return (
    <box 
      class="status-module audio" 
      spacing={4}
      $={(self) => {
        if (setup) setup(self)
        attachHoverDropdown(self, openDropdown, "audio")
      }}
    >
      <label label=" " class="icon" />
      <label label={audioInfo.as(i => i.vol)} />
    </box>
  )
}
  ...
  return (
    <box orientation={Gtk.Orientation.VERTICAL} spacing={8} class="popover-box">
      ...
      <box spacing={12}>
         ...
         <label label={audioInfo.as(i => i.vol)} class="detail-text" />
      </box>
      <box spacing={12}>
         <label label={audioInfo.as(i => i.micMuted ? "󰍭 Mic:" : "󰍬 Mic:")} class="section-title" />
         <label label={audioInfo.as(i => i.mic)} class="detail-text" />
      </box>
    </box>
  )
}
...
const cpuDetails = createPoll({ cores: "", top: "", freq: "" }, 5000, async () => {
  try {
...
export function CPU({ openDropdown, setup }: DropdownProps) {
  return (
    <box 
      class={cpuUsage.as(u => `status-module cpu ${u > 90 ? "critical" : u > 80 ? "warning" : ""}`)} 
      spacing={4}
      $={(self) => {
        if (setup) setup(self)
        attachHoverDropdown(self, openDropdown, "cpu")
      }}
    >
      <label label="" class="icon" />
      <label label={cpuUsage.as(u => `${Math.round(u)}%`)} />
    </box>
  )
}

export function CPUDropdown() {
  return (
    <box orientation={Gtk.Orientation.VERTICAL} spacing={10} class="popover-box">
      ...
      <box spacing={8}>
         ...
         <label label={cpuDetails.as(d => d.freq)} class="detail-text" />
      </box>
      <box orientation={Gtk.Orientation.VERTICAL} spacing={4}>
         <label label="PER CORE LOAD" class="section-title" halign={Gtk.Align.START} />
         <label label={cpuDetails.as(d => d.cores)} halign={Gtk.Align.START} class="detail-text" />
      </box>
      <box orientation={Gtk.Orientation.VERTICAL} spacing={4}>
         <label label="RESOURCE HOGS" class="section-title" halign={Gtk.Align.START} />
         <label label={cpuDetails.as(d => d.top)} halign={Gtk.Align.START} class="detail-text" />
      </box>
    </box>
  )
}
...
// RAM polls
const ramUsage = createPoll(0, 5000, async () => {
  const out = await execAsync("sh -c \"free -m | awk 'NR==2{print $3*100/$2}'\"").catch(() => "0")
  return parseFloat(out)
})

const ramDetails = createPoll({ top: "", swap: "", usedGb: "0GB" }, 5000, async () => {
  try {
...
export function RAM({ openDropdown, setup }: DropdownProps) {
  return (
    <box 
      class={ramUsage.as(u => `status-module ram ${u > 90 ? "critical" : u > 80 ? "warning" : ""}`)} 
      spacing={4}
      $={(self) => {
        if (setup) setup(self)
        attachHoverDropdown(self, openDropdown, "ram")
      }}
    >
      <label label="" class="icon" />
      <label label={ramUsage.as(u => `${Math.round(u)}%`)} />
    </box>
  )
}

export function RAMDropdown() {
  return (
    <box orientation={Gtk.Orientation.VERTICAL} spacing={10} class="popover-box">
      <label label="MEMORY" class="grid-header" />
      <box spacing={8}>
         <label label="Used RAM:" class="section-title" />
         <label label={ramDetails.as(d => d.usedGb)} class="detail-text" />
      </box>
      <box spacing={8}>
         <label label="Swap Usage:" class="section-title" />
         <label label={ramDetails.as(d => d.swap)} class="detail-text" />
      </box>
      <box orientation={Gtk.Orientation.VERTICAL} spacing={4}>
         <label label="TOP APPS" class="section-title" halign={Gtk.Align.START} />
         <label label={ramDetails.as(d => d.top)} halign={Gtk.Align.START} class="detail-text" />
      </box>
    </box>
  )
}
